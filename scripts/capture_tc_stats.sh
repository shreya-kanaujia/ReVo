...
attempts="${TC_CAPTURE_ATTEMPTS:-1200}"
...
for ((attempt = 1; attempt <= attempts; attempt++)); do
    ...
    qdisc_status=$?
    netem_snapshot="$(awk '/^qdisc netem /{capture=1} capture{print} capture && /^ backlog /{exit}' <<< "$qdisc_snapshot")"
    if [[ $qdisc_status -eq 0 && "$qdisc_snapshot" == *"qdisc htb 1:"* \
            && "$qdisc_snapshot" == *"qdisc netem 10:"* ]] \
        && grep -Eq 'Sent [1-9][0-9]* bytes [1-9][0-9]* pkt' <<< "$netem_snapshot"; then
        class_snapshot="$(tc -s class show dev "$interface" 2>&1)"
        class_status=$?
        {
            printf 'timestamp_utc=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
            printf 'interface=%s\n' "$interface"
            printf 'capture_exit_status=0\n'
